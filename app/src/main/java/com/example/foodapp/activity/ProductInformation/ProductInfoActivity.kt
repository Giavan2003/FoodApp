...
class ProductInfoActivity : AppCompatActivity() {
        ...
        FirebaseProductInfoHelper(productId).readComments(object : FirebaseProductInfoHelper.DataStatus {
            override fun DataIsLoaded(commentList: List<Comment>, count: Int, keys: List<String>) {
                binding.recComment.apply {
                    ...
                    adapter = CommentRecyclerViewAdapter(this@ProductInfoActivity, commentList, keys)
                }
    ...
    private fun updateCart(isCartExists: Boolean, isProductExists: Boolean, currentCart: Cart, currentCartInfo: CartInfo, amount: Int) {
        if (!isCartExists) {
            val cart = Cart(userId = userId, totalPrice = productPrice * amount, totalAmount = amount)
            val cartInfo = CartInfo(productId = productId, amount = amount)
            ...
            val cartInfo = CartInfo(productId = productId, amount = amount)
            currentCart.totalAmount += amount
            currentCart.totalPrice += amount * productPrice
            ...
            FailToast(this, "This product is already in the cart").showToast()
        }
    }
